Feature: Opel EPYX
  ...
  Background:
    * url OPEL_EPYX_URL
    * def testSuite = 'uat/Opel_EPYX'

  Scenario: 8.1.0 - GetJobDetails
    Given request read('uat/Opel_EPYX/requests/GetJobDetails.xml')
    When soap action
    Then status 200
  ...
  Scenario: 8.1.1 - GetSearchList
    Given request read('uat/Opel_EPYX/requests/GetSearchList.xml')
    When soap action
    Then status 200
    And match response /Envelope !contains 'Fault'
  ...
  Scenario: 8.1.2 - GetVehicleDetails
    Given request read('uat/Opel_EPYX/requests/GetVehicleDetails.xml')
    When soap action
    Then status 200
    And match response /Envelope !contains 'Fault'
